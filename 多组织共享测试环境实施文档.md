...
本地通过一台运行 **Ubuntu 24.04 LTS** 的专用服务器来执行测试任务。该服务器通过 USB 及网口与各种控制设备和测试设备相连，进而控制测试过程。整体集成测试环境为**单机架构**：

```
┌─────────────────────────────────────────────────────────────────┐
│                    测试服务器（Ubuntu 24.04 LTS）                 │
│  ┌─────────────┐  ┌─────────────┐  ┌─────────────────────────┐  │
│  │ GitHub      │  │ Runner      │  │ runner-wrapper          │  │
│  │ Actions     │  │ 容器/进程   │  │ （文件锁串行控制）       │  │
│  └──────┬──────┘  └──────┬──────┘  └───────────┬─────────────┘  │
│         │                │                      │                │
│         └────────────────┼──────────────────────┘                │
│                          │ USB / 网口                            │
...
**结论**：当前测试环境为单机部署，基于文件锁的 runner-wrapper 方案在架构上是匹配的。
...
- 已部署 [github-runners](https://github.com/yoinspiration/github-runners/tree/feat/runner-wrapper-multi-org-lock) 或手动注册的 GitHub Actions Runner
...
### 7.3 方案适用性分析与是否尝试其他方案

本方案**未完全解决**跨组织共享的所有问题（如 Discussion #346 所述：跨组织任务调度、统一任务队列仍依赖 workaround），但在当前单机测试环境下，**已可满足主要需求**。是否尝试其他方案，取决于实际场景。

#### 7.3.1 建议继续沿用当前方案的场景

在以下情况下，基于文件锁的 runner-wrapper 已足够：

| 条件 | 说明 |
|------|------|
| Runner 同机部署 | 所有 Runner 运行在同一台物理机（如当前 Ubuntu 24.04 LTS 测试服务器） |
| 硬件规模有限 | 开发板、串口、电源控制等资源数量可控 |
| 并发不密集 | 多组织同时触发 CI 的频率不高 |
| 无高级调度需求 | 不需要任务优先级、超时抢占等 |

**结论**：当前测试环境为单机架构（见第〇节），与上述条件基本吻合，现阶段可优先使用本方案，验证稳定后再考虑扩展。

#### 7.3.2 建议尝试其他方案的场景

| 需求 | 更适合的方案 | 说明 |
|------|--------------|------|
| **多机部署** | Redis / etcd 分布式锁 | 多台服务器共享同一套硬件时，文件锁无法跨机；可参考 Discussion #341 方案 3.1、3.3 |
| **任务优先级** | Jenkins / GitLab CI | 核心仓库需优先执行时，需支持优先级队列的 CI 平台 |
| **零维护、统一管理** | GitHub Enterprise | 有预算时可使用企业级 Runner Pool，原生支持跨组织调度 |
| **平台解耦** | 迁移至 GitLab CI / Jenkins | 可彻底脱离 GitHub Actions 的隔离限制 |

#### 7.3.3 推荐演进路径

1. **短期**：在单机环境下充分验证当前文件锁方案，观察多组织并发表现。
2. **中期**：若出现多机或调度冲突，优先考虑 **Redis 分布式锁方案**（思路与现有 wrapper 一致，仅将锁后端由文件改为 Redis）。
3. **长期**：若需任务优先级、超时等高级能力，再评估 GitHub Enterprise 或迁移至 GitLab/Jenkins。
...
**脚本实现**（[github-runners](https://github.com/yoinspiration/github-runners/tree/feat/runner-wrapper-multi-org-lock) 仓库）：
```
github-runners/
├── runner-wrapper/
│   ├── runner-wrapper.sh    # 文件锁包装脚本
│   └── README.md
└── Dockerfile               # 已集成 runner-wrapper
```
...
- [github-runners 仓库](https://github.com/yoinspiration/github-runners/tree/feat/runner-wrapper-multi-org-lock)
